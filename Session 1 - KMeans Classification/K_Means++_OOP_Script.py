...
class K_Means:
    ...
    def Fit(self, K_Means_plus_plus = True, num_repeats = 5):
        ...
        self.assignment_matrix = np.zeros([self.__N__,self.K])             
        ...
        for repeat in range(num_repeats):
            ...
            if K_Means_plus_plus:
                cluster_centres = np.array([random.choice(self.data)])
                for k in range(self.K - 1):
                    distance_values = [min([self.__distance_squared(self.data[n], cluster_centres[c]) for c in range(k + 1)]) for n in range(self.__N__)]
                    prob_dist = [distance_values[n] / sum(distance_values) for n in range(self.__N__)]
                    cummulative_prob = np.cumsum(prob_dist)
                    r = random.random()
                    for i, value in enumerate(cummulative_prob):
                        if value >= r: 
                            cluster_centres = np.append(cluster_centres, [self.data[i]], axis = 0)
                            break
            else:
            ...
            assignment_matrix = np.zeros([self.__N__,self.K])
            ...
            for i in range(self.iterations):
                ...
                for k in range(self.K):
                    ...
                    for d in range(self.__D__):
                        cluster_means.append(np.mean([self.data[n][d] for n in range(self.__N__) if assignment_matrix[n][k] == 1]))
            ...
            WSS_score = sum([self.__distance_squared(self.data[n],cluster_centres[k]) for n in range(self.__N__) for k in range(self.K) if assignment_matrix[n][k] == 1])
            ...
            if WSS_score < min_WSS_score:
                ...
                for k in range(self.K):
                    ...
                    for n in range(self.__N__):
                        self.assignment_matrix[n][k] = assignment_matrix[n][k]                    
    ...
    def Optimise_K(self, max_K = 7, num_repeats = 5):
        ...
        for k, value in enumerate(K_values):
            ...
            for n in range(kmeans_object.__N__):
                if kmeans_object.assignment_matrix[n][k] == 1:
    ...
    def plot_clustered_data(self):
        if self.__D__ == 2:
            try:
                for k in range(self.K):
                    cluster_points = np.transpose([self.data[n] for n in range(self.__N__) if self.assignment_matrix[n,k] == 1])
